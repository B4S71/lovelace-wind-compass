...
export class VacuumCard extends LitElement {
  ...
  static getStubConfig() {
    return {
      ...
      entity: "vacuum.robotic_vacuum_cleaner",
      start_button_entity: "input_button.start_vacuum_eg",
      automation_entity: "automation.bodenreinigung_erdgeschoss",
      schedule_time_entity: "input_datetime.staubsauger_start"
    };
  }

  static getConfigElement() {
    return document.createElement("slick-vacuum-card-editor");
  }
    ...
    try {
      ...
      if (this.hass.devices && this.hass.areas) {
          ...
          if (device && device.area_id) {
               ...
               if (area && area.name) {
                   ...
                   console.debug('VacuumCard: Discovered device area', this._deviceAreaName);
               }
          }
      }
      ...
      if (!this._discoveredBatteryEntity) {
          ...
          if (battCandidates.length > 0) {
              ...
              console.debug('VacuumCard: Discovered battery entity', best);
          }
      }

      // 4. Find room/zone sensor
      // Search for "room", "raum", "zone"
      if (!this._discoveredRoomEntity) {
           const roomCandidates = entities.filter(id => {
              ...
              return idLower.includes('room') || idLower.includes('raum') || idLower.includes('zone') || idLower.includes('aktueller_raum');
           });
           
           if (roomCandidates.length > 0) {
               this._discoveredRoomEntity = roomCandidates[0];
               console.debug('VacuumCard: Discovered room entity', this._discoveredRoomEntity);
           }
      }

    } catch (e) {
      console.warn('VacuumCard: Error discovering related entities', e);
    }
  }
  ...
  private _getActionMode(): 'start' | 'return' | 'none' {
    ...
    if (!vacuum) return 'none';
    
    // Cleaning or Paused -> Return
    if (['cleaning', 'error', 'paused'].includes(vacuum.state)) {
        return 'return';
    }
  ...
  private _handleStartClean(e?: Event) {
  ...
  render() {
    ...
    const vacuum = this.hass.states[this.config.entity];
    if (!vacuum) return html`<div>Vacuum not found</div>`;

    const stateStr = vacuum.state;
    ...
    // Room info (often in attributes, depending on integration)
    // Sometimes 'status' attribute gives better info
    const stateLabel = this._getStatusLabel(stateStr);
    let roomName: string | null = null;
    
    // Auto-discovered room sensor override (Active Room)
    if (this._discoveredRoomEntity) {
        const roomState = this.hass.states[this._discoveredRoomEntity];
        if (roomState && roomState.state && roomState.state !== 'unknown' && roomState.state !== 'unavailable') {
             roomName = roomState.state;
        }
    }
    
    // Fallback: If no active room, use Device Area
    if (!roomName && this._deviceAreaName) {
        roomName = this._deviceAreaName;
    }
    
    // If we have an active room name, we might want to append the floor/Area name if it's different 
    // But the requirement says "use the floor... if robot doesn't have a location entity".
    // Logic: 
    // 1. If we found a specific room sensor (e.g. "Kitchen"), use that.
    // 2. If NOT, use the device area (e.g. "Ground Floor").
    // Current logic does exactly that. 
    // However, if the user means "If the specific room sensor is empty/unknown, fall back to area", that is also handled.
    
    const cardTitle = attrs.friendly_name || this.config.entity;

    // Stats
    const area = attrs.cleaned_area ? `${attrs.cleaned_area} m²` : null;
    const time = attrs.cleaning_time ? `${attrs.cleaning_time} min` : null;
    ...
    // Top: Cleaning/Returning (Blue) vs Error (Red) vs Idle (Dark Greyish-Blue)
    let topColor = 'rgba(55, 71, 79, 0.25)'; // Dark Greyish-Blue (Idle, Docked, Paused, Unknown)
    
    if (['cleaning', 'returning'].includes(stateStr)) {
        topColor = 'rgba(33, 150, 243, 0.25)'; // Blue (Active)
      ...
      <div class="vacuum-card-container ${this._isTiny ? 'tiny' : (this._isShort ? 'short' : '')}" 
        ...
        <div class="header">
          ...
          <div class="info">
             ...
             ${!this._isShort ? html`
                 <div class="room">${stateLabel} ${roomName ? html`${roomName}` : ''}</div>
                 <div class="battery">
                   ${battery !== null ? html`<ha-icon icon="${batteryIcon}"></ha-icon> ${battery}%` : ''} 
                   ${(!roomName && attrs.fan_speed) ? `• ${attrs.fan_speed}` : ''}
                 </div>
        ...
        <div class="actions">
          ${actionMode !== 'none' && !this._isTriggered ? (this._isCompact ? html`
            <div class="action-btn ${actionMode === 'start' ? 'start-btn' : 'return-btn'}" 
                 @click=${actionMode === 'start' ? this._handleStartClean : this._handleReturnHome}>
          ...
          ` : html`
            <div class="swipe-slider ${actionMode === 'return' ? 'return-mode' : ''}">
               <div class="slide-track" style="opacity: ${Math.max(0, 1 - (this._swipeOffset / 150))}">
                   ${actionMode === 'start' ? 'Slide to Start' : 'Slide to Return'}
               </div>
...
export class VacuumCardEditor extends LitElement {
  ...
  render() { 
    ...
    const schema = [
         {name: "entity", label: "Vacuum Entity", selector: { entity: { domain: "vacuum" } }},
...
window.customCards.push({
    type: "slick-vacuum-card",
    name: "Slick Vacuum Card",
    preview: true,
    description: "A card for controlling a vacuum robot."
});
