  ...
  `;customElements.define("slick-notification-badge-card",St),customElements.define("slick-notification-badge-card-editor",class extends ot{static get properties(){return{hass:{attribute:!1},_config:{state:!0}}}setConfig(t){this._config=t}_valueChanged(t){const e={...this._config,...t.detail.value},i=new CustomEvent("config-changed",{detail:{config:e},bubbles:!0,composed:!0});this.dispatchEvent(i)}render(){if(!this.hass||!this._config)return U``;return U`
    ...
    `}}),window.customBadges=window.customBadges||[],window.customBadges.push({type:"slick-notification-badge-card",name:"Slick Notification Badge Card",preview:!0,description:"Displays badges for calendar events and active entities."});console.info("%c SLICK-LIGHT-CONTROL-CARD %c 0.1.0 ","color: white; background: #FFC107; font-weight: 700;","color: #FFC107; background: white; font-weight: 700;");customElements.define("slick-light-control-card",class extends ot{constructor(){super(...arguments),this._interacting=!1,this._interactionMode="light",this._interactState=null,this._pointerStartTime=0,this._pointerStartX=0,this._pointerStartY=0}static get properties(){return{hass:{attribute:!1},config:{state:!0},_interacting:{state:!0}}}static getConfigElement(){return document.createElement("slick-light-control-card-editor")}static getStubConfig(){return{type:"custom:slick-light-control-card",entity:"light.example_light",name:"Living Room",covers:[]}}setConfig(t){if(!t.entity)throw new Error("You need to define an entity (light)");this.config=t}shouldUpdate(t){if(t.has("config"))return!0;const e=t.get("hass");if(!e)return!0;if(e.states[this.config.entity]!==this.hass.states[this.config.entity])return!0;if(this.config.covers)for(const t of this.config.covers)if(e.states[t]!==this.hass.states[t])return!0;return!1}_toggleLight(t){t&&t.stopPropagation(),this.hass.callService("light","toggle",{entity_id:this.config.entity})}_controlCover(t,e){this.hass.callService("cover",e,{entity_id:t})}_handlePointerDown(t){var e,i,n;const s=t.composedPath();for(const t of s)if((null===(e=t.classList)||void 0===e?void 0:e.contains("control-btn"))||(null===(i=t.classList)||void 0===i?void 0:i.contains("icon-container")))return;this._interacting=!0,this._pointerStartTime=Date.now(),this._pointerStartX=t.clientX,this._pointerStartY=t.clientY;const a=null===(n=this.shadowRoot)||void 0===n?void 0:n.querySelector("ha-card");if(a){const e=a.getBoundingClientRect(),i=(t.clientY-e.top)/e.height;this.config.covers&&this.config.covers.length>0&&i>.65?this._interactionMode="cover":this._interactionMode="light",a.setPointerCapture(t.pointerId)}}_handlePointerUp(t){var e;if(!this._interacting)return;this._interacting=!1;const i=null===(e=this.shadowRoot)||void 0===e?void 0:e.querySelector("ha-card");if(i){i.releasePointerCapture(t.pointerId);const e=Date.now()-this._pointerStartTime,n=Math.sqrt(Math.pow(t.clientX-this._pointerStartX,2)+Math.pow(t.clientY-this._pointerStartY,2));e<250&&n<10?"light"===this._interactionMode&&this._toggleLight():"light"===this._interactionMode?this._applyLightState(t,i):this._applyCoverState(t,i)}}_handlePointerMove(t){var e;if(!this._interacting)return;const i=null===(e=this.shadowRoot)||void 0===e?void 0:e.querySelector("ha-card");if(i){const e=i.getBoundingClientRect(),n=Math.max(0,Math.min(1,(t.clientX-e.left)/e.width)),s=Math.max(0,Math.min(1,(t.clientY-e.top)/e.height));if("light"===this._interactionMode){const t=Math.round(100*(1-s));this._interactState={main:`Brightness: ${t}%`}}else{let t="";if(n<.1)t="Open";else if(n>.9)t="Closed";else{t=`${Math.round(100*(1-n))}%`}const e=Math.round(100*(1-s));this._interactState={main:`Position: ${t}`,secondary:`Tilt: ${e}%`}}}}_applyCoverState(t,e){const i=e.getBoundingClientRect(),n=Math.max(0,Math.min(1,(t.clientX-i.left)/i.width)),s=Math.max(0,Math.min(1,(t.clientY-i.top)/i.height));(this.config.covers||[]).forEach(t=>{if(n<.1)this.hass.callService("cover","open_cover",{entity_id:t});else if(n>.9)this.hass.callService("cover","close_cover",{entity_id:t});else{const e=Math.round(100*(1-n));this.hass.callService("cover","set_cover_position",{entity_id:t,position:e})}if(s>=0&&s<=1){const e=Math.round(100*(1-s));this.hass.callService("cover","set_cover_tilt_position",{entity_id:t,tilt_position:e})}})}_applyLightState(t,e){const i=e.getBoundingClientRect(),n=Math.max(0,Math.min(1,(t.clientX-i.left)/i.width)),s=Math.max(0,Math.min(1,(t.clientY-i.top)/i.height)),a=this.hass.states[this.config.entity];if(!a)return;const o=Math.round(100*(1-s)),r=a.attributes.min_mireds||153,c=a.attributes.max_mireds||500,l=Math.round(r+n*(c-r)),d={entity_id:this.config.entity,brightness_pct:Math.max(1,o)};a.attributes.min_mireds&&(d.color_temp=l),this.hass.callService("light","turn_on",d)}render(){const t=this.hass.states[this.config.entity];if(!t)return U`<ha-card class="not-found">Entity not found: ${this.config.entity}</ha-card>`;const e="on"===t.state,i=t.attributes.brightness?Math.round(t.attributes.brightness/255*100):0,n=t.attributes.min_mireds||153,s=t.attributes.max_mireds||500,a=t.attributes.color_temp||Math.round((n+s)/2),o=this.config.covers||[];let r="background: linear-gradient(135deg, #2c3e50 0%, #000000 100%);";if(this._interacting)if("cover"===this._interactionMode){r=`background: ${"linear-gradient(to bottom, rgba(255,255,255,0) 0%, rgba(0,0,0,0.4) 100%)"}, ${"linear-gradient(to bottom, \n            transparent 0%, \n            rgba(0,0,0,0.1) 4%, transparent 4%,\n            transparent 8%, \n            rgba(0,0,0,0.15) 12%, transparent 12%,\n            transparent 15%,\n            rgba(0,0,0,0.2) 19%, transparent 19%,\n            transparent 22%,\n            rgba(0,0,0,0.2) 26%, transparent 26%,\n            transparent 29%,\n            rgba(0,0,0,0.25) 34%, transparent 34%,\n            transparent 37%,\n            rgba(0,0,0,0.25) 42%, transparent 42%,\n            transparent 45%,\n            rgba(0,0,0,0.3) 51%, transparent 51%,\n            transparent 54%,\n            rgba(0,0,0,0.3) 60%, transparent 60%,\n            transparent 63%,\n            rgba(0,0,0,0.35) 70%, transparent 70%,\n            transparent 73%,\n            rgba(0,0,0,0.35) 81%, transparent 81%,\n            transparent 84%,\n            rgba(0,0,0,0.4) 93%, transparent 93%,\n            transparent 96%,\n            rgba(0,0,0,0.4) 100%\n           )"}, ${"linear-gradient(to right, #87CEEB 0%, #333333 100%)"};`}else r="background: linear-gradient(to bottom, rgba(0,0,0,0) 0%, rgba(0,0,0,1) 100%), linear-gradient(to right, rgb(166, 209, 255) 0%, rgb(255, 160, 0) 100%);";else if(e){const e=(a-n)/(s-n);if(t.attributes.rgb_color){const[e,i,n]=t.attributes.rgb_color;r=`background: linear-gradient(135deg, rgba(${e},${i},${n},0.8) 0%, rgba(${e},${i},${n},0.4) 100%);`}else{const t=200+55*e,i=220-20*e,n=255-105*e;r=`background: linear-gradient(135deg, rgb(${t},${i},${n}) 0%, rgb(${Math.round(.6*t)},${Math.round(.6*i)},${Math.round(.6*n)}) 100%);`}}const c=this._interacting&&"cover"===this._interactionMode;return U`
      ...
      >
        ...
        ${c&&this._interactState?U`
        <div class="status-container">
            <div class="status-badge">${this._interactState.main}</div>
            ${this._interactState.secondary?U`<div class="status-badge">${this._interactState.secondary}</div>`:""}
        </div>
        `:""}
      </ha-card>
